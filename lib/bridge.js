...
var construct = function () {
  ...
  var Bridge = function (config) {
    ...
    if (!config.sdbWrapper) {
      throw new Error('Bridge must be initialised with an sdbWrapper instance');
    }
   ...
   * uninstall was OK
   */
  Bridge.prototype.uninstall = function (packageName, stopOnFailure, cb) {
    var logger = this.logger;

    this.runTizenAppScript('uninstall', [packageName], function (err, stdout, stderr) {
      logger.write(stdout);

      if (err || stdout.match('not installed|failed')) {
        if (stopOnFailure) {
          logger.error(stderr);
          cb(new Error('package ' + packageName + ' could not be uninstalled'));
        }
        else {
